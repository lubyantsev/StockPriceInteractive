...
def main():
    ...
    6. Рассчитывает дополнительные индикаторы, такие как RSI и MACD.
    7. Рассчитывает стандартное отклонение и строит полосы Боллинджера.
    8. Строит и сохраняет график цен закрытия и скользящих средних.
    9. Уведомляет пользователя о сильных колебаниях цен, если они превышают заданный порог.
    10. Рассчитывает и отображает среднюю цену закрытия.
    11. Запрашивает у пользователя имя файла для экспорта данных и выполняет экспорт в CSV.
    12. Вызывает функцию для построения интерактивного графика с передачей указанных дат.
    """
    ...
    try:
        ...
        plot_interactive_graph(stock_data, ticker, period, start_date, end_date)
